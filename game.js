...
function renderTravel() {
  ...
  if (state.phase === 'map') {
    ...
    $('travel-surface').innerHTML =
      title('游商的行囊', `现有 ${state.gold} 金币。标记的商品可补足队伍搭配；可以买多件，也可以不买，商品不会刷新。`) +
...
function showForge() {
  showDialog(
    'forge',
    `${heading('把这次停留，变成锋芒', '精制一件装备，加成提升 60%；本次营地不再恢复远征生命。', 'CAMP WORKSHOP')}<div class="choices">${E.campOptions(
      state,
    )
      .map(
        o =>
...
function renderStatus() {
  ...
  text('life-status', state.life > 60 ? '状态良好' : state.life > 30 ? '稍显疲惫' : '亟需休整');
  text('gold', state.gold);
  text('population', `${roster.length} / ${state.capacity}`);
  $('income').innerHTML =
  ...
  $('synergies').innerHTML =
    ...
    `<div class="role-traits">${Object.entries(E.ROLE_TRAITS)
      ...
      .join('')}</div>`;
  text('relic-count', state.relics.length);
  ...
  text(
    'status-toggle',
    `${expanded ? '收起详情' : '羁绊与行囊'} · ${Object.keys(E.FACTIONS).filter(f => t[f] >= 2).length} 组羁绊 · ${state.bag.length} 件装备 ${expanded ? '▴' : '▾'}`,
  );
  $('status-toggle').setAttribute('aria-expanded', String(expanded));
}
function renderScouting() {
  const current = E.currentNode(state),
    node = state.phase === 'map' ? selectedMapNode() : current,
  ...
  text('enemy-tag', `第 ${node.act + 1} 章 · ${E.NODES[node.kind].name}`);
  text('enemy-name', node.kind === 'event' && state.phase === 'map' ? '未知事件' : node.name);
  text('enemy-description', nodeSummary(node));
  $('enemy-list').innerHTML = foes
    .map(
      u =>
        `<${state.phase === 'map' ? 'div' : 'button'} class="enemy-row" ${state.phase === 'map' ? '' : `data-inspect="${u.id}" title="查看 ${E.TYPES[u.type].name}"`}>${art(u.type)}<span>${E.TYPES[u.type].name}<small>${factionName(u.type)} · ${E.ROLES[E.TYPES[u.type].role]}</small></span><span class="enemy-meta">${'★'.repeat(u.star)}</span></${state.phase === 'map' ? 'div' : 'button'}>`,
    )
    .join('');
  $('scout-tip').innerHTML =
...
function renderCombatFocus() {
  ...
  lines.innerHTML = '';
  if (u && target) {
    const a = effectPos(u.pos),
      v = effectPos(target.pos),
      dx = v.x - a.x,
      dy = v.y - a.y,
...
function renderSelection() {
  ...
  if (mode && u.pos !== null) {
    ...
    if (['guard', 'knight', 'oakmaul', 'emberguard'].includes(u.type))
      area = all.filter(p => E.distance(p, u.pos) <= (u.type === 'guard' ? 3 : 1));
    else if (u.type === 'ancient') area = all;
    else if (['healer', 'oracle', 'warden', 'pearl', 'tideguard', 'songbird'].includes(u.type))
      area = liveUnits()
        .filter(v => v.side === u.side)
        .map(v => v.pos);
      ...
      area = all.filter(p => E.distance(p, aim) <= (['mage', 'frost', 'hexer'].includes(u.type) ? 1 : 0));
  }
  $('board')
...
function renderControls() {
  ...
  let label = '开始战斗 →',
    disabled = !n;
  if (state.phase === 'map') {
    disabled = !E.availableNodes(state).some(n => n.id === selectedMapNode().id);
  ...
  text('speed', '×' + (prefs.speed || 1));
  text('sound', prefs.enabled ? '♫' : '♪');
...
function render() {
  ...
  if (state.battle && ['battle', 'result'].includes(state.phase)) {
    ...
    text('enemy-power', `${E.enemyRoster(state).length} 人 · 战力 ${power(E.enemyRoster(state), [], enemyScale())}`);
    text('formation-power', `${E.deployed(state).length} 人 · 战力 ${power(E.deployed(state), state.relics)}`);
  }
...
function spellMark(pos, type) {
  ...
  const mark =
    glyphs[type] ||
    glyphs[{ oakmaul: 'hunter', duskblade: 'rogue', sparkscout: 'mage', cinder: 'breaker', flarebow: 'hunter' }[type]];
  if (!mark) return;
...
function animateEvents(events) {
  ...
  for (const ev of events) {
    ...
    if (ev.type === 'cast') {
      ...
      if (
        !['healer', 'oracle', 'warden', 'knight', 'pearl', 'tideguard', 'songbird', 'emberguard'].includes(ev.unitType)
      )
        ring(
...
function recordEnd() {
  ...
  try {
    ...
    notify('记录暂未写入浏览器，请保留此页并稍后重试。', true);
  }
}
...
function openSharedRun() {
  ...
  if (!config) return;
  if (config.error) notify(config.error, true);
  ...
  showDialog(
    ...
    `${heading(enemies ? '前方的对手' : '森林伙伴图鉴', '了解技能，再选择适合你的伙伴。', 'COMPANIONS & ABILITIES')}${
      ...
      types
        .map(id => {
          const d = E.TYPES[id];
          return `<article class="codex-card">${art(id)}<h3>${d.name}</h3><small>${factionName(id)} · ${E.ROLES[d.role]} · ${d.cost ? '◈ ' + d.cost : '首领'}</small><p>♥ ${d.hp} · ⚔ ${d.atk} · 护甲 ${d.armor}<br>攻击间隔 ${d.interval}s · 射程 ${d.range}</p><small>✧ ${d.skill}</small><p>${d.desc}</p></article>`;
        })
...
function showTrait(id) {
  const f = E.FACTIONS[id],
    ...
    types = Object.keys(E.TYPES).filter(t => E.TYPES[t].cost > 0 && (f ? E.hasFaction(t, id) : E.TYPES[t].role === id));
  showDialog(
    'trait',
...
document.addEventListener('click', event => {
  ...
  if (d.buy !== undefined) {
    action(E.buy(state, Number(d.buy)), 'buy');
    return;
  }
  if (d.cell !== undefined) {
    moveTo(Number(d.cell));
    return;
  }
  if (d.actor) {
    selectActor(d.actor);
  ...
  if (d.bench !== undefined) {
    if (d.bench) selectActor(d.bench);
    else moveTo(null);
    return;
  }
  ...
  if (d.mapAct !== undefined) {
    showMap(Number(d.mapAct));
    return;
  }
  if (d.camp) {
    action(E.camp(state, d.camp), 'reward');
    return;
  }
  if (d.forge) {
  ...
  if (d.merchant !== undefined) {
    action(E.merchantBuy(state, d.merchant), 'buy', '商品已收入行囊。');
    return;
  }
  if (d.eventChoice) {
    action(E.takeEvent(state, d.eventChoice), 'reward');
    return;
  }
  if (d.treasure) {
    action(E.treasure(state, d.treasure), 'reward');
    return;
  }
  ...
  switch (button.id) {
    ...
    case 'speed':
      ...
      break;
    case 'auto':
      action(E.autoDeploy(state), 'move', '已按前排承伤、后排输出安排队伍。');
      break;
    case 'refresh':
      action(E.refresh(state), 'click');
      break;
    case 'lock':
      if (E.canManage(state)) {
        state.locked = !state.locked;
        action({}, 'click', state.locked ? '下场战斗将保留这批酒馆伙伴。' : '已解除酒馆锁定。');
      }
      break;
    case 'expand':
      action(E.expand(state), 'buy', '人口已扩充，可以再上阵一位伙伴。');
      break;
    case 'restart':
    case 'new-from-end':
      showNewRun();
    ...
    case 'sound':
      ...
      notify(prefs.enabled ? '音效已开启' : '音效已关闭');
      break;
    case 'preview-sound':
      if (prefs.enabled) audio.play('victory');
      else notify('先开启音效，再试听。');
      break;
    case 'dismiss-tip':
      prefs.tip = false;
      savePrefs();
