...
async function main() {
  ...
  const target = await (
    await fetch('http://127.0.0.1:41879/json/new?' + encodeURIComponent('about:blank'), { method: 'PUT' })
  ).json();
  ...
  try {
    ...
    for (let i = 0; i < 40; i++) {
      const ready = await call('Runtime.evaluate', {
        expression: "document.readyState==='complete'&&!!document.querySelector('[data-actor]')",
        returnByValue: true,
      });
      if (ready.result.value) break;
      if (i === 39) throw Error('Game failed to render');
    ...
    const geometry = await call('Runtime.evaluate', {
      expression: '({width:innerWidth,scroll:document.documentElement.scrollWidth,height:innerHeight})',
      returnByValue: true,
    });
    console.log(JSON.stringify({ file: out, ...geometry.result.value }));
  } finally {
    ws.close();
    await fetch('http://127.0.0.1:41879/json/close/' + target.id);
  }
}
main().catch(e => {
