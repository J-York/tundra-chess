      ...
      `${heading(enemies ? '前方的对手' : '森林伙伴图鉴', '了解技能，再选择适合你的伙伴。', 'COMPANIONS & ABILITIES')}${
        full
          ? `<div class="codex-filters"><label>阵营<select id="codex-faction"><option value="all">所有阵营</option>${Object.entries(
              ...
              .join(
                '',
              )}</select></label><small>显示 ${types.length} / ${Object.values(E.TYPES).filter(d => d.cost).length} 位</small></div><p class="tiny muted">阵营 2 / 3 / 4 档，职业 2 / 3（守卫、法师、辅助、先知到 4；狂战仅 2 / 3）档，可以交叉搭配；双阵营伙伴同时计入两边，纹章还能为一位伙伴额外接上一个阵营。每个角色仍只有一个主动技能。</p>`
          : ''
